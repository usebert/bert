...
export function CalendarItemDetails({ item, canManage, userEmail, saving, onEdit, onComplete, onArchive }: Props) {
  const badge = calendarStatusBadge(item.status);
      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {item.priority === "high" ? (
              <span className="rounded bg-red-50 px-1.5 py-0.5 text-[11px] font-medium text-red-700">High priority</span>
            ) : null}
          </div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            {item.assignedPersonName || item.assignedPersonEmail || "Unassigned"}
