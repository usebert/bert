...
import {
  buildNcrListItems,
  buildNcrSummaryMetrics,
  filterItemsForTab,
  filterNcrItems,
  sortNcrItems,
} from "./adapters/ncrListAdapter";
import { NcrFilters } from "./components/NcrFilters";
import { NcrList } from "./components/NcrList";
import { NcrSummaryCards } from "./components/NcrSummaryCards";
import type { NcrFilterState, NcrWorkspaceProps, NcrWorkspaceTab } from "./types";

const TAB_LABELS: Record<NcrWorkspaceTab, string> = {
  open: "Open",
  overdue: "Overdue",
...
export function NcrWorkspace({
  ...
  const [activeTab, setActiveTab] = useState<NcrWorkspaceTab>("open");
  const [filters, setFilters] = useState<NcrFilterState>({ query: "", site: "", severity: "", status: "" });
  ...
  const summaryMetrics = useMemo(() => buildNcrSummaryMetrics(visible), [visible]);
  const listItems = useMemo(() => {
    const items = buildNcrListItems(visible);
    const tabbed = filterItemsForTab(items, activeTab);
    const filtered = filterNcrItems(tabbed, filters);
    return sortNcrItems(filtered);
  }, [visible, activeTab, filters]);

  const tabs: NcrWorkspaceTab[] = canArchiveNcr
    ? ["open", "overdue", "awaiting-verification", "closed", "archived"]
    : ["open", "overdue", "awaiting-verification", "closed"];

  return (
    <PageContainer>
      <PageHeader
        eyebrow={t("ncrs.title")}
        title={t("ncrs.subtitle")}
        description="Non-conformance register with containment, cause analysis, corrective action, and verification."
        secondaryActions={
