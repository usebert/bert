#!/usr/bin/env node
/**
 * Production Reporting workflow smoke test.
 */
import dotenv from "dotenv";
import { createFetchTransport } from "./lib/production-auth-health-core.mjs";
import {
  formatReportingWorkflowReport,
  loadReportingWorkflowConfig,
  REPORTING_VERIFIER_BUDGET_MS,
  runProductionReportingWorkflowChecks,
} from "./lib/production-reporting-workflow-core.mjs";

dotenv.config();

let interruptCleanup = null;
let shuttingDown = false;

async function handleShutdownSignal(signal) {
  if (shuttingDown) {
    return;
  }
  shuttingDown = true;
  console.error(`\n[reporting] Received ${signal} — attempting verification-only cleanup...`);
  try {
    if (typeof interruptCleanup === "function") {
      const cleanupResult = await interruptCleanup();
      console.error(`[reporting] Interrupt cleanup: ${JSON.stringify(cleanupResult?.results || cleanupResult)}`);
    }
  } catch (error) {
    console.error(`[reporting] Interrupt cleanup failed: ${error instanceof Error ? error.message : String(error)}`);
  }
  process.exit(signal === "SIGINT" ? 130 : 143);
}

for (const signal of ["SIGINT", "SIGTERM"]) {
  process.on(signal, () => {
    void handleShutdownSignal(signal);
  });
}

async function main() {
  const config = loadReportingWorkflowConfig();
  if (config.missing.length > 0) {
    console.error(
      [
        "Missing required smoke-test environment variables:",
        ...config.missing.map((key) => `  - ${key}`),
        "",
        "Example:",
        "  BERT_SMOKE_USERNAME=mr.important \\",
        "  BERT_SMOKE_PASSWORD='<set securely in your environment>' \\",
        "  BERT_SMOKE_COMPANY_FOLDER_ID=... \\",
        "  BERT_SMOKE_MASTER_SHEET_ID=... \\",
        "  BERT_SMOKE_ALLOW_REPORT_MUTATION=1 \\",
        "  npm run verify:production-reporting-workflow",
      ].join("\n"),
    );
    process.exit(1);
  }

  const runId = Date.now();
  console.log(
    `[reporting] Starting production Reporting verifier (budget ${REPORTING_VERIFIER_BUDGET_MS}ms, run bert-smoke-report-${runId}).`,
  );

  const transport = createFetchTransport(config.apiBase, config.appOrigin, config.timeoutMs);
  const result = await runProductionReportingWorkflowChecks(config, transport, {
    runId,
    registerInterruptCleanup(fn) {
      interruptCleanup = fn;
    },
  });

  console.log(formatReportingWorkflowReport(result));
  process.exit(result.ok ? 0 : 1);
}

main().catch((error) => {
  console.error(`[reporting] Fatal error: ${error instanceof Error ? error.message : String(error)}`);
  process.exit(1);
});
